...
def test_CanAddProduct():
    items = {'qnt': 10, 'unit_price': 20, 'discount': 5}
    invoice = Invoice()
    assert invoice.addProduct(10, 20, 5) == items

def test_DiscountFormula(products):
    invoice = Invoice()
    assert invoice.discountFormula(products) == 5.625
